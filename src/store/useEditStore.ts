  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (!state) return
        const has9 = state.manuals.some(m => m.id === "9")
        if (!has9) {
          const newManual = DEFAULT_MANUALS.find(m => m.id === "9")
          if (newManual) {
            state.manuals = [...state.manuals, newManual]
          }
        }
      },
    }

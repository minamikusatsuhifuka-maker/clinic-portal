          ...
          <div className="p-5 space-y-5">
            ...
            <div>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                <h4 style={{ fontSize:13, fontWeight:700, color:"var(--text-primary)" }}>📌 初動対応フロー</h4>
                <div style={{ display:"flex", gap:6 }}>
                  ...
                  <button onClick={() => { setEditFlowSteps(flow.map(s => ({ ...s }))); setEditingFlow(!editingFlow) }}
                    style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:"#7c65cc", border:"1px solid rgba(124,101,204,0.25)", background:"var(--subtle-bg)", padding:"4px 10px", borderRadius:8, cursor:"pointer" }}>
              ...
              {editingFlow ? (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {editFlowSteps.map((step, i) => (
                    <div key={i} style={{ background:"var(--subtle-bg)", borderRadius:12, padding:12, border:"1px solid rgba(124,101,204,0.15)" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                        ...
                        <input value={step.title}
                          ...
                          style={{ flex:1, border:"1px solid rgba(124,101,204,0.2)", borderRadius:8, padding:"6px 10px", fontSize:12, fontWeight:600, color:"var(--text-primary)", background:"var(--surface-bg)", outline:"none", fontFamily:"inherit" }} />
                        <div style={{ display:"flex", gap:4 }}>
                          <button onClick={() => { if(i > 0){ const s=[...editFlowSteps]; [s[i-1],s[i]]=[s[i],s[i-1]]; setEditFlowSteps(s) } }}
                            style={{ width:24, height:24, borderRadius:6, border:"1px solid rgba(124,101,204,0.2)", background:"var(--surface-bg)", cursor:"pointer", fontSize:12, color:"#7c65cc" }}>↑</button>
                      ...
                      <textarea value={step.desc}
                        ...
                        rows={2}
                        style={{ width:"100%", border:"1px solid rgba(124,101,204,0.2)", borderRadius:8, padding:"6px 10px", fontSize:12, color:"var(--text-secondary)", background:"var(--surface-bg)", outline:"none", resize:"none", fontFamily:"inherit", lineHeight:1.6 }} />
                    </div>
                  ))}
            ...
            <div>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                <h4 style={{ fontSize:13, fontWeight:700, color:"var(--text-primary)" }}>📞 緊急連絡先</h4>
                ...
                {contacts.map((c, i) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:12, background:"var(--subtle-bg)", borderRadius:12, padding:"10px 14px" }}>
                    ...
                    {c.phone !== "未定" ? (
                      ...
                      <span style={{ fontSize:11, color:"var(--text-secondary)", background:"white", border:"1px solid rgba(124,101,204,0.15)", padding:"6px 12px", borderRadius:10 }}>後日設定</span>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                <h4 style={{ fontSize:13, fontWeight:700, color:"var(--text-primary)" }}>☑ 初動チェックリスト</h4>
...
export default function RiskPage({ userRole }: { userRole?: string }) {
  ...
  return (
    <div style={{ padding:24, maxWidth:800 }}>
      ...
      {/* リスト */}
      <div style={{ background:"var(--surface-bg)", borderRadius:20, border:"1px solid rgba(124,101,204,0.11)", overflow:"hidden", boxShadow:"0 2px 8px rgba(90,60,160,0.06)" }}>
        {filtered.map((r, idx) => (
          <motion.button key={r.id}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:11, color:"var(--text-secondary)", marginTop:3 }}>{r.summary}</div>
            </div>

...
import { X, Send, ChevronRight, Phone, Pencil } from "lucide-react"
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <motion.div initial={{ scale: 0.94, opacity: 0, y: 12 }}
          animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.94, opacity: 0, y: 12 }}
          ...
          <div className="sticky top-0 bg-white/95 backdrop-blur-sm px-6 py-5 border-b border-violet-100 flex items-start justify-between rounded-t-3xl z-10">
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-full bg-violet-50 hover:bg-violet-100 flex items-center justify-center text-violet-400 flex-shrink-0 ml-2">
              <X size={15} />
            </button>
          </div>
          <div className="p-6 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-bold text-violet-800 text-sm">📞 緊急連絡先</h4>
                <button onClick={() => setEditingContact(true)}
                  className="flex items-center gap-1.5 text-xs text-violet-500 hover:text-violet-700 border border-violet-200 hover:border-violet-400 px-2.5 py-1 rounded-lg transition-colors bg-violet-50">
                  <Pencil size={11} />編集
                </button>
              </div>

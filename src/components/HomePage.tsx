...
const card:React.CSSProperties={background:"var(--surface-bg)",borderRadius:16,border:"1px solid rgba(124,101,204,0.11)",boxShadow:"0 1px 4px rgba(90,60,160,0.05)",padding:16}
  ...
  return (
    <motion.div variants={cont} initial="hidden" animate="show" style={{padding:24,maxWidth:900,display:"flex",flexDirection:"column",gap:14}}>
      <motion.div variants={item} style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
        <div>
          <h1 style={{fontSize:20,fontWeight:700,color:"var(--text-primary)"}}>おはようございます 🌸</h1>
          <p style={{fontSize:12,color:"var(--text-secondary)",marginTop:3}}>{dateStr}</p>
        </div>
        ...
        <motion.div variants={item}
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:13, fontWeight:600, color:"var(--text-primary)" }}>{latestMsg.title}</div>
              <div style={{ fontSize:12, color:"var(--text-secondary)", marginTop:2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{latestMsg.body.split("\n")[0]}</div>
            </div>
          </div>
        </motion.div>
      )}
      ...
      <motion.div variants={item} style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
        {[
          ...
          <motion.button key={s.label} whileHover={{scale:1.02,y:-2}} whileTap={{scale:0.98}} onClick={()=>setActivePage(s.page)} style={{...card,textAlign:"left",cursor:"pointer",border:"1px solid rgba(124,101,204,0.11)"}}>
            ...
            <div style={{fontSize:26,fontWeight:700,color:"var(--text-primary)",lineHeight:1}}>{s.val}<span style={{fontSize:12,fontWeight:400,color:"var(--text-secondary)",marginLeft:4}}>{s.unit}</span></div>
          </motion.button>
        ))}
      </motion.div>
      ...
      <motion.div variants={item} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        <div style={card}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12,paddingBottom:10,borderBottom:"1px solid rgba(124,101,204,0.08)"}}>
            <CheckCircle2 size={14} style={{color:"#a78bfa"}}/><span style={{fontSize:13,fontWeight:600,color:"var(--text-primary)",flex:1}}>本日のチェックリスト</span><span style={{fontSize:11,color:"var(--text-secondary)"}}>4/7</span>
          </div>
        ...
        <div style={card}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:12,paddingBottom:10,borderBottom:"1px solid rgba(124,101,204,0.08)"}}>
            <Bell size={14} style={{color:"#a78bfa"}}/><span style={{fontSize:13,fontWeight:600,color:"var(--text-primary)"}}>Chatwork 最近の通知</span>
          </div>
          {[
            ...
            <div key={n.title} style={{background:n.bg,border:`1px solid ${n.border}`,borderRadius:10,padding:"9px 12px",marginBottom:7,display:"flex",gap:10}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:"var(--text-secondary)",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{n.body}</div>
              </div>
              <span style={{fontSize:10,color:"var(--text-secondary)",flexShrink:0}}>{n.time}</span>
            </div>
          ))}
        </div>
      </motion.div>

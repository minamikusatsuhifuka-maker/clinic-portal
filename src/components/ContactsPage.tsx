...
const card: React.CSSProperties = { background:"var(--surface-bg)", borderRadius:16, border:"1px solid rgba(124,101,204,0.11)", boxShadow:"0 1px 4px rgba(90,60,160,0.05)" }
const inp: React.CSSProperties = { width:"100%", border:"1px solid rgba(124,101,204,0.2)", borderRadius:10, padding:"9px 12px", fontSize:13, color:"var(--text-primary)", background:"var(--subtle-bg)", outline:"none", fontFamily:"inherit" }
...
const CAT_COLOR: Record<ContactCategory, { color: string; bg: string }> = {
  ...
  "その他":         { color:"var(--text-secondary)", bg:"#f8f6fc" },
}
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.5)", backdropFilter:"blur(4px)" }}>
      <motion.div initial={{ scale:0.9, opacity:0 }} animate={{ scale:1, opacity:1 }} style={{ ...card, width:"100%", maxWidth:420, padding:24 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
          ...
          <div><div style={{ fontSize:15, fontWeight:700, color:"var(--text-primary)" }}>重複の可能性があります</div><div style={{ fontSize:12, color:"var(--text-secondary)", marginTop:2 }}>類似した連絡先がすでに登録されています</div></div>
        </div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
          <div style={{ background:"var(--subtle-bg)", borderRadius:12, padding:12 }}><div style={{ fontSize:11, fontWeight:700, color:"var(--text-secondary)", marginBottom:6 }}>既存の連絡先</div><div style={{ fontSize:13, fontWeight:600, color:"var(--text-primary)" }}>{existing.name||existing.company}</div><div style={{ fontSize:12, color:"var(--text-secondary)" }}>{existing.phone}</div></div>
          <div style={{ background:"#fff0f0", borderRadius:12, padding:12, border:"1px solid #fca5a5" }}><div style={{ fontSize:11, fontWeight:700, color:"#f87171", marginBottom:6 }}>新しい連絡先</div><div style={{ fontSize:13, fontWeight:600, color:"var(--text-primary)" }}>{incoming.name||incoming.company}</div><div style={{ fontSize:12, color:"var(--text-secondary)" }}>{incoming.phone}</div></div>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={onCancel} style={{ flex:1, padding:"10px", borderRadius:12, border:"1px solid rgba(124,101,204,0.2)", background:"var(--surface-bg)", color:"var(--text-secondary)", fontSize:13, fontWeight:600, cursor:"pointer" }}>登録しない</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.4)", backdropFilter:"blur(4px)" }} onClick={e => { if (e.target===e.currentTarget) onClose() }}>
      <motion.div initial={{ scale:0.94, opacity:0 }} animate={{ scale:1, opacity:1 }} style={{ ...card, width:"100%", maxWidth:520, maxHeight:"92vh", overflowY:"auto" }}>
        <div style={{ padding:"16px 20px", borderBottom:"1px solid rgba(124,101,204,0.1)", display:"flex", justifyContent:"space-between", alignItems:"center", position:"sticky", top:0, background:"var(--surface-bg)", zIndex:1 }}>
          <div style={{ fontSize:14, fontWeight:700, color:"var(--text-primary)" }}>{initial?.name ? "連絡先を編集" : "連絡先を追加"}</div>
        ...
        <div style={{ padding:20, display:"flex", flexDirection:"column", gap:12 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>氏名</label><input style={inp} value={form.name} onChange={e => s("name",e.target.value)} placeholder="楠葉 展大"/></div>
            <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>会社・組織名</label><input style={inp} value={form.company} onChange={e => s("company",e.target.value)} placeholder="南草津皮フ科"/></div>
            <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>部署</label><input style={inp} value={form.department} onChange={e => s("department",e.target.value)} placeholder="診療部"/></div>
            <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>役職</label><input style={inp} value={form.role} onChange={e => s("role",e.target.value)} placeholder="院長"/></div>
            <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>電話番号 <span style={{ color:"#f472b6" }}>*</span></label><input style={inp} value={form.phone} onChange={e => s("phone",e.target.value)} placeholder="077-599-1451"/></div>
            <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>電話番号2</label><input style={inp} value={form.phone2} onChange={e => s("phone2",e.target.value)} placeholder="090-XXXX-XXXX"/></div>
          </div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>メールアドレス</label><input style={inp} value={form.email} onChange={e => s("email",e.target.value)} placeholder="info@example.com"/></div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>住所</label><input style={inp} value={form.address} onChange={e => s("address",e.target.value)} placeholder="滋賀県草津市..."/></div>
          <div>
            <label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:6 }}>カテゴリ</label>
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {CATEGORIES.map(cat => { const cc=CAT_COLOR[cat]; return (<button key={cat} onClick={() => s("category",cat)} style={{ fontSize:11, padding:"5px 12px", borderRadius:999, border:`1.5px solid ${form.category===cat?cc.color:"rgba(124,101,204,0.15)"}`, background:form.category===cat?cc.bg:"#fff", color:form.category===cat?cc.color:"var(--text-secondary)", cursor:"pointer", fontWeight:form.category===cat?700:400 }}>{cat}</button>) })}
            </div>
          </div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>どんなときに連絡するか</label><textarea style={{ ...inp, resize:"none" }} rows={2} value={form.whenToContact} onChange={e => s("whenToContact",e.target.value)} placeholder="例：医療事故発生時・法的問題が発生したとき"/></div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"var(--text-secondary)", display:"block", marginBottom:4 }}>メモ</label><textarea style={{ ...inp, resize:"none" }} rows={2} value={form.notes} onChange={e => s("notes",e.target.value)} placeholder="担当者名・対応時間・注意事項など"/></div>
      ...
      <div onDragOver={e => { e.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={onDrop} onClick={() => inputRef.current?.click()}
        ...
        {loading ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
            ...
            <div style={{ fontSize:14, fontWeight:600, color:"var(--text-primary)" }}>名刺・連絡先をドロップ</div>
            <div style={{ fontSize:12, color:"var(--text-secondary)", lineHeight:1.6 }}>またはクリックしてファイルを選択<br/>JPG・PNG・PDF対応 / 携帯で撮影した写真もOK</div>
      ...
      <div
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            <span style={{ fontSize:13, fontWeight:700, color:"var(--text-primary)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{contact.name||contact.company}</span>
            {contact.role && <span style={{ fontSize:11, color:"var(--text-secondary)", flexShrink:0 }}>{contact.role}</span>}
          </div>
          {contact.name && contact.company && <div style={{ fontSize:11, color:"var(--text-secondary)", marginTop:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{contact.company}{contact.department?` · ${contact.department}`:""}</div>}
        </div>
        ...
        {contact.phone && <a href={`tel:${contact.phone}`} onClick={e => e.stopPropagation()} style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, fontWeight:600, color:"#5f4ba8", textDecoration:"none", flexShrink:0, padding:"4px 10px", borderRadius:8, background:"var(--subtle-bg)" }}><Phone size={12} style={{ color:"#a78bfa" }}/>{contact.phone}</a>}
        <div style={{ display:"flex", gap:3, flexShrink:0 }} onClick={e => e.stopPropagation()}>
          ...
          <button onClick={onEdit} style={{ width:26, height:26, borderRadius:6, background:"transparent", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", color:"var(--text-secondary)" }}><Edit3 size={11}/></button>
        ...
        {expanded && hasDetails && (
          <motion.div initial={{ height:0, opacity:0 }} animate={{ height:"auto", opacity:1 }} exit={{ height:0, opacity:0 }} style={{ overflow:"hidden" }}>
            <div style={{ padding:"0 16px 12px 64px", display:"flex", flexDirection:"column", gap:5 }}>
              {contact.whenToContact && <div style={{ fontSize:12, color:"var(--text-secondary)", display:"flex", gap:6 }}><span style={{ color:"#a78bfa", fontWeight:600, flexShrink:0 }}>連絡タイミング:</span>{contact.whenToContact}</div>}
              {contact.phone2 && <div style={{ fontSize:12, color:"var(--text-secondary)", display:"flex", alignItems:"center", gap:5 }}><Phone size={11} style={{ color:"var(--text-secondary)" }}/>電話2: {contact.phone2}</div>}
              ...
              {contact.address && <div style={{ fontSize:12, color:"var(--text-secondary)", display:"flex", alignItems:"center", gap:5 }}><MapPin size={11} style={{ color:"var(--text-secondary)" }}/>{contact.address}</div>}
              {contact.notes && <div style={{ fontSize:12, color:"var(--text-secondary)", fontStyle:"italic" }}>{contact.notes}</div>}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <div style={{ padding:24, maxWidth:860 }}>
      ...
      {filtered.length>0 && (
        <div style={{ ...card, padding:0, overflow:"hidden" }}>
          ...
          {others.length>0 && (
            <>
              {favs.length>0 && <div style={{ padding:"10px 16px", background:"var(--subtle-bg)", borderTop:"1px solid rgba(124,101,204,0.07)", borderBottom:"1px solid rgba(124,101,204,0.07)", fontSize:12, fontWeight:700, color:"var(--text-secondary)" }}>その他の連絡先</div>}
      ...
      {filtered.length===0 && <div style={{ ...card, padding:48, textAlign:"center" }}><div style={{ fontSize:36, marginBottom:12 }}>📞</div><div style={{ fontSize:14, fontWeight:600, color:"var(--text-secondary)", marginBottom:6 }}>連絡先が見つかりません</div><div style={{ fontSize:12, color:"var(--text-secondary)" }}>「AI自動読み取り」で名刺の写真をアップロードするか、手動で追加してください</div></div>}

    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background:"rgba(0,0,0,0.4)", backdropFilter:"blur(4px)" }} onClick={e => { if (e.target===e.currentTarget) onClose() }}>
      <motion.div initial={{ scale:0.94, opacity:0 }} animate={{ scale:1, opacity:1 }} style={{ ...card, width:"100%", maxWidth:500, maxHeight:"90vh", overflowY:"auto" }}>
        <div style={{ padding:"18px 20px", borderBottom:"1px solid rgba(124,101,204,0.1)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ fontSize:14, fontWeight:700, color:"#1e2230" }}>💡 気づきを記録する</div>
        ...
        <div style={{ padding:20, display:"flex", flexDirection:"column", gap:14 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <div><label style={{ fontSize:11, fontWeight:600, color:"#6b7280", display:"block", marginBottom:5 }}>名前</label><input style={inp} value={form.authorName} onChange={e => setForm(p => ({ ...p, authorName:e.target.value }))}/></div>
            <div><label style={{ fontSize:11, fontWeight:600, color:"#6b7280", display:"block", marginBottom:5 }}>職種</label><select style={inp} value={form.authorRole} onChange={e => setForm(p => ({ ...p, authorRole:e.target.value }))}>{ROLES.map(r => <option key={r}>{r}</option>)}</select></div>
          </div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"#6b7280", display:"block", marginBottom:5 }}>学びのきっかけ</label><select style={inp} value={form.source} onChange={e => setForm(p => ({ ...p, source:e.target.value }))}>{SOURCES.map(s => <option key={s}>{s}</option>)}</select></div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"#6b7280", display:"block", marginBottom:5 }}>何を学んだか・気づいたか <span style={{ color:"#f472b6" }}>*</span></label><textarea style={{ ...inp, resize:"none" }} rows={3} value={form.learning} onChange={e => setForm(p => ({ ...p, learning:e.target.value }))} placeholder="例：感謝を言葉にして伝えることで、相手との信頼関係が深まることを学んだ。"/></div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"#6b7280", display:"block", marginBottom:5 }}>どう実践するか（具体的な行動）</label><textarea style={{ ...inp, resize:"none" }} rows={2} value={form.practice} onChange={e => setForm(p => ({ ...p, practice:e.target.value }))} placeholder="例：今日から毎日1人のスタッフに、具体的な感謝の言葉を伝える。"/></div>
          <div><label style={{ fontSize:11, fontWeight:600, color:"#6b7280", display:"block", marginBottom:5 }}>実践した結果（後で記入してもOK）</label><textarea style={{ ...inp, resize:"none" }} rows={2} value={form.result} onChange={e => setForm(p => ({ ...p, result:e.target.value }))} placeholder="例：チームの雰囲気が明るくなってきた気がする。"/></div>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <span style={{ fontSize:11, color:"#6b7280" }}>共有するとチームの学びになります</span>
          </div>
...
export default function InsightPage({ userRole="staff", userName="スタッフ" }: { userRole?: string; userName?: string }) {
  ...
  return (
    <div style={{ padding:24, maxWidth:800 }}>
      ...
      {filtered.length===0 ? (
        <div style={{ ...card, padding:48, textAlign:"center" }}><div style={{ fontSize:36, marginBottom:12 }}>💡</div><div style={{ fontSize:14, fontWeight:600, color:"#6b7280", marginBottom:6 }}>気づきがまだ記録されていません</div><div style={{ fontSize:12, color:"#6b7280", lineHeight:1.7 }}>研修・日常業務・書籍から得た気づきを記録して<br/>学びを行動に変え、チームで共有しましょう</div></div>
      ) : (
          ...
          {filtered.map((item,i) => (
            <motion.div key={item.id} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:i*0.04 }} style={{ ...card, padding:18, borderLeft:"4px solid #818cf8" }}>
              <div style={{ display:"flex", alignItems:"flex-start", gap:10, marginBottom:12 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:4 }}>
                    <span style={{ fontSize:12, fontWeight:600, color:"#1e2230" }}>{item.authorName}</span>
                    <span style={{ fontSize:11, color:"#6b7280" }}>{item.authorRole}</span>
                    ...
                    {item.isShared ? <span style={{ fontSize:10, color:"#22c55e", display:"flex", alignItems:"center", gap:3 }}><Globe size={10}/>共有中</span> : <span style={{ fontSize:10, color:"#6b7280", display:"flex", alignItems:"center", gap:3 }}><Lock size={10}/>非公開</span>}
                  </div>
                  <div style={{ fontSize:11, color:"#6b7280" }}>{new Date(item.createdAt).toLocaleDateString("ja-JP",{ month:"long", day:"numeric" })}</div>
                </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:12 }}>
                <div style={{ background:"#f0ede8", borderRadius:10, padding:"10px 12px" }}><div style={{ fontSize:11, fontWeight:700, color:"#7c65cc", marginBottom:4 }}>📖 学んだこと・気づき</div><p style={{ fontSize:13, color:"#1e2230", margin:0, lineHeight:1.7 }}>{item.learning}</p></div>
                {item.practice && <div style={{ background:"#edfbf4", borderRadius:10, padding:"10px 12px" }}><div style={{ fontSize:11, fontWeight:700, color:"#166534", marginBottom:4 }}>🚀 実践すること</div><p style={{ fontSize:13, color:"#1e2230", margin:0, lineHeight:1.7 }}>{item.practice}</p></div>}
                {item.result ? (
                  <div style={{ background:"#f0f7ff", borderRadius:10, padding:"10px 12px" }}><div style={{ fontSize:11, fontWeight:700, color:"#1d4ed8", marginBottom:4 }}>✨ 実践した結果</div><p style={{ fontSize:13, color:"#1e2230", margin:0, lineHeight:1.7 }}>{item.result}</p></div>
                ) : item.authorName===userName ? (
                  <textarea rows={1} placeholder="実践した結果を記録しましょう..." defaultValue={item.result} onBlur={e => { if (e.target.value!==item.result) updateInsight(item.id,{ result:e.target.value }) }} style={{ width:"100%", border:"1px dashed rgba(124,101,204,0.3)", borderRadius:10, padding:"8px 12px", fontSize:12, color:"#1e2230", background:"#fafafa", resize:"none", outline:"none", fontFamily:"inherit" }}/>
                ) : null}
              </div>
              {isManager && <div style={{ marginBottom:10 }}><textarea rows={1} placeholder="マネージャーコメント・フィードバック..." defaultValue={item.managerComment} onBlur={e => { if (e.target.value!==item.managerComment) updateInsight(item.id,{ managerComment:e.target.value }) }} style={{ width:"100%", border:"1px solid rgba(167,139,250,0.2)", borderRadius:10, padding:"8px 12px", fontSize:12, color:"#1e2230", background:"#f0ede8", resize:"none", outline:"none", fontFamily:"inherit" }}/></div>}
              {!isManager && item.managerComment && <div style={{ background:"#f0ede8", borderRadius:10, padding:"8px 12px", marginBottom:10 }}><div style={{ fontSize:11, fontWeight:700, color:"#7c65cc", marginBottom:3 }}>👑 マネージャーより</div><p style={{ fontSize:12, color:"#1e2230", margin:0, lineHeight:1.6 }}>{item.managerComment}</p></div>}
